...
void loop(){
    ...
    for(int j=0; j<6; j++){
      ...
      if((yaw[j] < SERVOMAX) && (yaw[j] > SERVOMIN)) pwm.setPWM(3*j, 0, yaw[j]);
      if((pitch[j] < SERVOMAX) && (pitch[j] > SERVOMIN)) pwm.setPWM(3*j+1, 0, pitch[j]);
      if((bend[j] < SERVOMAX) && (bend[j] > SERVOMIN)) pwm.setPWM(j+2+2, 0, bend[j]);
	  }
